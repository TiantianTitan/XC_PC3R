...
short src_ln3 [] = {
	  0,  83,  85,  82,  87,  82,  87,   0, };
S_F_MAP src_file3 [] = {
	{ "-", 0, 0 },
	{ "./tme4_parite1.pml", 1, 6 },
	{ "-", 7, 8 }
};
uchar reached3 [] = {
	  0,   1,   0,   0,   1,   1,   0,   0, };
...
short src_ln2 [] = {
	  0,  66,  69,  71,  72,  74,  68,  76, 
	 65,  77,  65,  77,   0, };
S_F_MAP src_file2 [] = {
	{ "-", 0, 0 },
	{ "./tme4_parite1.pml", 1, 11 },
	{ "-", 12, 13 }
};
uchar reached2 [] = {
	  0,   1,   1,   0,   1,   0,   0,   1, 
...
short src_ln1 [] = {
	  0,  40,  42,  43,  44,  45,  45,  41, 
	 47,  47,  49,  50,  51,  53,  54,  55, 
	 56,  56,  52,  58,  48,  59,  39,  60, 
	 39,  60,   0, };
S_F_MAP src_file1 [] = {
	{ "-", 0, 0 },
	{ "./tme4_parite1.pml", 1, 25 },
	{ "-", 26, 27 }
};
uchar reached1 [] = {
	  0,   1,   1,   0,   0,   1,   1,   0, 
...
short src_ln0 [] = {
	  0,  25,  27,  28,  29,  30,  31,  31, 
	 26,  33,  24,  34,  24,  34,   0, };
S_F_MAP src_file0 [] = {
	{ "-", 0, 0 },
	{ "./tme4_parite1.pml", 1, 13 },
	{ "-", 14, 15 }
};
uchar reached0 [] = {
	  0,   1,   1,   0,   0,   0,   1,   1, 
...
{	int j = 0, h = now._nr_pr;
	...
	case 3:	/* Collecteur */
		...
		((P3 *)pptr(h))->id = 0;
#ifdef VAR_RANGES
		logval("Collecteur:statut", ((P3 *)pptr(h))->statut);
		logval("Collecteur:id", ((P3 *)pptr(h))->id);
#endif
#ifdef HAS_CODE
		locinit3(h);
#endif
	...
	case 2:	/* Ouvrier */
		...
		((P2 *)pptr(h))->id = 0;
#ifdef VAR_RANGES
		logval("Ouvrier:statut", ((P2 *)pptr(h))->statut);
		logval("Ouvrier:id", ((P2 *)pptr(h))->id);
#endif
#ifdef HAS_CODE
		locinit2(h);
#endif
	...
	case 1:	/* Gestionnaire */
		...
		((P1 *)pptr(h))->id = 0;
#ifdef VAR_RANGES
		logval("Gestionnaire:statut", ((P1 *)pptr(h))->statut);
		logval("Gestionnaire:id", ((P1 *)pptr(h))->id);
#endif
#ifdef HAS_CODE
		locinit1(h);
#endif
	...
	case 0:	/* Producteur */
		...
		((P0 *)pptr(h))->id = 1;
#ifdef VAR_RANGES
		logval("Producteur:id", ((P0 *)pptr(h))->id);
#endif
#ifdef HAS_CODE
		locinit0(h);
#endif
...
{
		now.prodToGest = addqueue(calling_pid, 1, 1);
		now.gestToOuv = addqueue(calling_pid, 2, 1);
		now.ouvToGest = addqueue(calling_pid, 3, 1);
		now.ouvToColl = addqueue(calling_pid, 4, 1);
		now.gestQueueSize = 0;
#ifdef VAR_RANGES
...
{	int i = 0;
	...
	{	if (t->ty[i] == TIMEOUT_F) return 1;
		...
		switch (t->qu[i]) {
		case 0: break;
		case 1: if (	(t->ty[i] == Q_FULL_F && ( q_full((int) ( now.prodToGest) )))
			 || 	(t->ty[i] == Q_FULL_T && (!q_full((int) ( now.prodToGest) )))
			 || 	(t->ty[i] == Q_EMPT_F && ( !q_len((int) ( now.prodToGest) )))
			 || 	(t->ty[i] == Q_EMPT_T && (  q_len((int) ( now.prodToGest) )))
			    ) return 0; break;
		case 2: if (	(t->ty[i] == Q_FULL_F && ( q_full((int) ( now.gestToOuv) )))
			 || 	(t->ty[i] == Q_FULL_T && (!q_full((int) ( now.gestToOuv) )))
			 || 	(t->ty[i] == Q_EMPT_F && ( !q_len((int) ( now.gestToOuv) )))
			 || 	(t->ty[i] == Q_EMPT_T && (  q_len((int) ( now.gestToOuv) )))
			    ) return 0; break;
		case 3: if (	(t->ty[i] == Q_FULL_F && ( q_full((int) ( now.ouvToGest) )))
			 || 	(t->ty[i] == Q_FULL_T && (!q_full((int) ( now.ouvToGest) )))
			 || 	(t->ty[i] == Q_EMPT_F && ( !q_len((int) ( now.ouvToGest) )))
			 || 	(t->ty[i] == Q_EMPT_T && (  q_len((int) ( now.ouvToGest) )))
			    ) return 0; break;
		case 4: if (	(t->ty[i] == Q_FULL_F && ( q_full((int) ( now.ouvToColl) )))
			 || 	(t->ty[i] == Q_FULL_T && (!q_full((int) ( now.ouvToColl) )))
			 || 	(t->ty[i] == Q_EMPT_F && ( !q_len((int) ( now.ouvToColl) )))
			 || 	(t->ty[i] == Q_EMPT_T && (  q_len((int) ( now.ouvToColl) )))
			    ) return 0; break;
...
{	/* int i; */
	...
	printf("	mtype  C:	2\n");
	printf("	mtype  DONE:	1\n");
	printf("	chan prodToGest (=%d):	len %d:\t", now.prodToGest, q_len(now.prodToGest));
	c_chandump(now.prodToGest);
	printf("	chan gestToOuv (=%d):	len %d:\t", now.gestToOuv, q_len(now.gestToOuv));
	c_chandump(now.gestToOuv);
	printf("	chan ouvToGest (=%d):	len %d:\t", now.ouvToGest, q_len(now.ouvToGest));
	c_chandump(now.ouvToGest);
	printf("	chan ouvToColl (=%d):	len %d:\t", now.ouvToColl, q_len(now.ouvToColl));
	c_chandump(now.ouvToColl);
	printf("	int    gestQueueSize:	%d\n", now.gestQueueSize);
}
void
...
{	/* int i; */
	...
	printf("	mtype  statut:	%d\n", ((P3 *)pptr(pid))->statut);
	printf("	int    id:	%d\n", ((P3 *)pptr(pid))->id);
		break;
	case 2:
		printf("local vars proc %d (Ouvrier):\n", pid);
	printf("	mtype  statut:	%d\n", ((P2 *)pptr(pid))->statut);
	printf("	int    id:	%d\n", ((P2 *)pptr(pid))->id);
		break;
	case 1:
		printf("local vars proc %d (Gestionnaire):\n", pid);
	printf("	mtype  statut:	%d\n", ((P1 *)pptr(pid))->statut);
	printf("	int    id:	%d\n", ((P1 *)pptr(pid))->id);
		break;
	case 0:
		printf("local vars proc %d (Producteur):\n", pid);
	printf("	int    id:	%d\n", ((P0 *)pptr(pid))->id);
		break;
	}
}
...
printm(int x, char *s)
{
	...
	switch (x) {
	case 1: Printf("DONE"); return;
	case 2: Printf("C"); return;
	case 3: Printf("R"); return;
	case 4: Printf("V"); return;
